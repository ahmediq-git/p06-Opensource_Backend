...
function insertSingleFieldDocTest() {
  ...
  let id;
  eb.database
    .insertSingleFieldDoc("newcol1", { name: "sample" })
    .then((ans) => {
      id = ans;
      return id;
    })
...
function insertDocTest() {
  console.log(
    "Testing of insertDoc (Doc should be of arbitrary fields), it returns doc id"
  );
  eb.database.insertDoc("newcol1", { name: "A", age: 25 }).then((id) => {
    console.log(id);
  });
  console.log("");
}
...
function deleteDocTest(id) {
  ...
  eb.database
    .getAllDocs("newcol1")
    .then((ans) => console.log("after deletion of a doc", ans));
}
//7 insertManyFields
...
function insertManyDocsTest() {
  ...
  eb.database
    .insertManyDocs("newcol2", [
      { name: "A", age: 20 },
      { name: "B" },
      { name: "C" },
    ])
    .then((resp) => console.log("We got response", resp));
  eb.database
    .getAllDocs("newcol2")
    .then((ans) => console.log("after inserting many docs", ans));
}
...
function getCollectionNamesTest() {
  console.log(
    "Testing getting names of all Collections (should return all collection names)"
  );
